...
def detect(save_img=False):
    ...
    sideBySide = not opt.not_side_by_side
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if img.ndimension() == 3:
            img = img.unsqueeze(0)
        # Inference
        result = predictAndBound(model, img, im0s, webcam, path, dataset, save_dir,
                                 names, save_txt, save_img, view_img, vid_cap, colors, vid_path, vid_writer, modelName)
        if floatModel is not None:
            result2 = predictAndBound(floatModel, img, im0s, webcam, path, dataset, save_dir,
                                      names, save_txt, save_img, view_img, vid_cap, colors, vid_path, vid_writer,
                                      "float")
            if view_img:
                numpy_horizontal = np.hstack((result, result2))
                cv2.imshow('annotated', numpy_horizontal)
        elif view_img:
            cv2.imshow('annotated', result)
...
def predictAndBound(model, img, im0s, webcam, path, dataset, save_dir, names, save_txt, save_img, view_img, vid_cap,
                    colors, vid_path, vid_writer,
                    modelName):
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        print(f'{s}Done {modelName}.  ({t2 - t1:.3f}s)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--not-side-by-side', action='store_true', help='dont show results for '
                                                                        'both quantized and float model')
