...
def test(data,
    ...
    quantizationDataLoader = torch.utils.data.DataLoader(quantizationDataset, batch_size=batch_size,
    ...
    if not training:
        ...
        dataloader = create_dataloader(path, imgsz, batch_size, maxStride, opt, pad=0.5, rect=True,
    ...
    if len(stats) and stats[0].any():
        ...
        nt = np.bincount(stats[3].astype(np.int32), minlength=nc)  # number of targets per class
    else:
        nt = torch.zeros(1)
